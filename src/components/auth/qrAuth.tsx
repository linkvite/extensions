...
export function AuthWithQR({ onLogin }: { onLogin: OnLogin }) {
	...
	async function init() {
		...
		const data = (await res.json()) as {
			data: { token: string; endpoint: string };
		};
		...
		<AuthQrContainer>
			<QRContainer $qr={!qr.loading && !qr.owner}>
				{qr.loading ? (
					...
					<QRAuthImage src={qr.owner.avatar} alt="qr-avatar" />
				) : qr.code ? (
					<AuthQrCode qrCode={qr.code} />
				) : (
			...
			<QRSubTitle isSubText>
				{qr.owner
					? `Trying to login as @${qr.owner.username}?`
					: qr.loading
						? "Trying to get a QR code, please wait..."
						: qr.code
