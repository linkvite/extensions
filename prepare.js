...
const popupJSFile = fs.readdirSync(`${root}/`).find((file) => file.startsWith('popup.') && file.endsWith('.js'));
const popupCSSFile = fs.readdirSync(`${root}/`).find((file) => file.startsWith('popup.') && file.endsWith('.css'));
...
if (popupCSSFile) {
    files.push({ from: `${root}/${popupCSSFile}`, to: `${destination}/index.css` });
}
